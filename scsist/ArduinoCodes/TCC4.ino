...
int meio = 10;
int atras = 11;
String codigo = String(2); //string for fetching data from address
	...
	{    //testa se o existe
		...
		{    //testa se o cliente esta conectado
			...
			{    //retorna o numero de bytes no buffer de entrada         
				char c = client.read();
                                //le char a char e armazena na string codigo
                                if (codigo.length() < 2)
                                {
                                    //Armazena caracteres na string codigo
                                    codigo += c;
                                }
				switch(codigo[0]){
				...
				delay(1); // da um tempo para  o cliente receber os caracteres 
			}
		}
